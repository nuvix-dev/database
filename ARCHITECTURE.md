...
Enforcement happens _inside_ generated SQL: the context's roles feed permission predicates into WHERE clauses, so a read can never return rows the caller cannot see. Internal operations pass `SYSTEM_CONTEXT` explicitly down their own call chains rather than granting broad roles.
